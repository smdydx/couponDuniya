...
def upgrade() -> None:
    ...
    # Add created_at to order_items to support partitioning (only if table exists)
    if table_exists(connection, 'order_items'):
        op.add_column("order_items", sa.Column("created_at", sa.DateTime(), nullable=False, server_default=sa.func.now()))

    # Partitioning via trigger-based inheritance (only if offer_clicks table exists)
    ...
    # Only create order_items partition function if table exists
    if table_exists(connection, 'order_items'):
        op.execute(
            """
